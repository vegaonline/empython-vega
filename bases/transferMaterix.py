...
class IsotropicTranferMatrix(TransferMatrix):
    ...
    def solve(self, wls):
        ...
        def find_roots(wl, epsilon, alpha, beta):
            ...
            coeff = [K[2, 2], alpha * (K[0, 2] + K[2, 0]) + beta * (K[1, 2] + K[2, 1]),
                     alpha2 * (K[0, 0] + K[2, 2]) + alpha * beta * (K[1, 0] + K[0, 1]) +
                     beta2 * (K[1, 1] + K[2, 2]) +
                     (K[0, 2] * K[2, 0] + K[1, 2] * K[2, 1] - K[0, 0] * K[2, 2] - K[1, 1] * K[2, 2]),
                     alpha3 * (K[0, 2] + K[2, 0]) + beta3 * (K[1, 2] + K[2, 1]) +
                     alpha2 * beta * (K[1, 2] + K[2, 1]) + alpha * beta2 * (K[0, 2] + K[2, 0]) +
                     alpha * (K[0, 1] * K[1, 2] + K[1, 0] * K[2, 1] - K[0, 2] * K[1, 1] - K[2, 0] * K[1, 1]) +
                     beta * (K[0, 1] * K[2, 0] + K[1, 0] * K[0, 2] - K[0, 0] * K[1, 2] - K[0, 0] * K[2, 1]),
                     alpha4 * (K[0, 0]) + beta4 * (K[1, 1]) + alpha3 * beta * (K[0, 1] + K[1, 0]) +
                     alpha * beta3 * (K[0, 1] + K[1, 0]) + alpha2 * beta2 * (K[0, 0] + K[1, 1]) +
                     ...
                     K[0, 0] * K[1, 1] * K[2, 2] - K[0, 0] * K[1, 2] * K[2, 1] - K[1, 0] * K[0, 1] * K[2, 2] +
                     K[1, 0] * K[0, 2] * K[2, 1] + K[2, 0] * K[0, 1] * K[1, 2] - K[2, 0] * K[0, 2] * K[1, 1]]
        ...
        for iwl, wl in enumerate(self.wls):
            ...
            r = S.array([[M[1, 0] * M[2, 2] - M[1, 2] * M[2, 0], M[3, 0] * M[2, 2] - M[3, 2] * M[2, 0]],
                         [M[0, 0] * M[1, 2] - M[1, 0] * M[0, 2], M[0, 0] * M[3, 2] - M[3, 0] * M[0, 2]]],
    ...
    def __str__(self):
        return 'ANISOTROPIC TRANSFER MATRIX SOLVER\n\n%s\n\ntheta inc x = %g\ntheta inc y = %g' % (self.multilayer.__str__(), self.theta_inc_x, self.theta_inc_y)
